...
class _BirdPainter extends CustomPainter {
  ...
  const _BirdPainter({required this.wingAngle});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawOval(
      Rect.fromCenter(center: Offset(cx, cy), width: r * 3.0, height: r * 1.2),
      bodyPaint,
    );

    // Wing (animated)
    final wingPath = Path();
    final wingY = cy + sin(wingAngle) * r * 0.5;
    wingPath.moveTo(cx - r * 0.2, cy);
    wingPath.quadraticBezierTo(cx - r * 0.8, wingY, cx - r * 0.2, cy + r * 0.3);
    canvas.drawPath(
      wingPath,
      Paint()
    ...
    // Beak
    final beakPath = Path();
    beakPath.moveTo(cx + r * 0.65, cy + r * 0.05);
    beakPath.lineTo(cx + r * 1.1, cy + r * 0.18);
    beakPath.lineTo(cx + r * 0.65, cy + r * 0.32);
    beakPath.close();
    canvas.drawPath(
      beakPath,
      Paint()
        ...
        ).createShader(Rect.fromLTWH(cx + r * 0.6, cy, r * 0.6, r * 0.4)),
    );

    // Beak line
    canvas.drawLine(
      Offset(cx + r * 0.65, cy + r * 0.18),
      Offset(cx + r * 1.08, cy + r * 0.18),
      Paint()
    ...
    canvas.drawOval(
      Rect.fromCenter(
        center: Offset(cx - r * 0.15, cy - r * 0.35),
        width: r * 0.7,
        height: r * 0.4,
      ),
  ...
  @override
  bool shouldRepaint(_BirdPainter old) => old.wingAngle != wingAngle;
}
